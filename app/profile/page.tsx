...
export default function ProfilePage() {
  ...
  async function uploadAvatar() {
    ...
    setBusyUpload(true);
    try {
      const url = await uploadAvatarFile(me.uid, file); // sends contentType
  ...
  async function removeAvatar() {
    ...
    setBusyUpload(true);
    try {
      // delete everything under avatars/{uid}
  ...
  return (
    <main className="profilePage">
      ...
      <style jsx>{`
        ...
        .avatarColumn { display: grid; gap: 12px; align-content: start; }
        .avatarWrap {
          position: relative;
          width: 140px;
          height: 140px;
        ...
        .avatarTrigger {
          ...
          border: none; background: rgba(15,23,42,.65); color: #fff;
          font-size: 12px; font-weight: 600; padding: 6px 0; cursor: pointer;
        }
        .avatarActions { display: flex; gap: 10px; flex-wrap: wrap; }
        .avatarHint { font-size: 12px; }
        .heroInfo { display: grid; gap: 14px; align-content: start; }
        .eyebrow { font-size: 12px; letter-spacing: .18em; text-transform: uppercase; color: var(--muted); }
        .heroInfo h1 { margin: 0; font-size: clamp(28px, 4vw, 36px); letter-spacing: -0.02em; }
        .heroInfo p { margin: 0; color: var(--muted); font-size: 14px; max-width: 420px; }
        .heroMeta { display: flex; gap: 10px; flex-wrap: wrap; font-size: 13px; color: var(--muted); }
        .heroMeta span { border: 1px solid var(--border); padding: 6px 10px; border-radius: 999px; background: var(--bg2); }
        .heroButtons { display: flex; gap: 12px; flex-wrap: wrap; }
        .profileGrid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 18px; }
        .profileCard { display: grid; gap: 16px; }
        .span2 { grid-column: span 2; }
        .cardTitle { margin: 0; font-size: 20px; font-weight: 700; }
        .cardSubtitle { margin: -8px 0 0; color: var(--muted); font-size: 13px; }
        ...
        .chip { border: 1px solid var(--border); background: var(--bg2); color: var(--text); border-radius: 999px; padding: 6px 12px; cursor: pointer; font-weight: 600; }
        ...
        .actionsRow { display: flex; justify-content: flex-end; gap: 10px; flex-wrap: wrap; }
        .themeBlock { padding: 12px 0; border-bottom: 1px dashed var(--border); }
        .providerRow { display: flex; gap: 8px; flex-wrap: wrap; }
        .providerPill { padding: 4px 10px; border-radius: 999px; border: 1px solid var(--border); font-size: 12px; }
        .dangerCard { border: 1px solid color-mix(in oklab, #ef4444 35%, var(--border)); background: color-mix(in oklab, #ef4444 4%, var(--card-bg)); }
        .ov { position: fixed; inset: 0; background: rgba(2,6,23,.55); display: grid; place-items: center; padding: 16px; z-index: 2200; }
        .box { width: 100%; max-width: 520px; background: var(--card-bg); border-radius: 18px; border: 1px solid var(--border); overflow: hidden; box-shadow: 0 40px 60px rgba(15,23,42,.35); }
        .bh { display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--border); padding: 12px 16px; }
        .bt { font-weight: 800; }
        .x { border: none; background: transparent; color: var(--muted); font-size: 20px; cursor: pointer; }
        .body { padding: 16px; display: grid; gap: 12px; }
        @media (max-width: 900px) {
          .heroCard { grid-template-columns: 1fr; text-align: center; }
          .avatarColumn { justify-items: center; }
          .heroInfo { text-align: center; }
          .heroButtons { justify-content: center; }
        }
        @media (max-width: 768px) {
          .profilePage { padding: 16px 0 96px; }
